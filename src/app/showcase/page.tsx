      ...
      <main className="min-h-screen bg-neutral-950 text-white">
        ...
        <section className="relative px-4 py-16 lg:py-24">
          <div className="max-w-6xl mx-auto text-center">
            ...
            {/* Title */}
            <h1 className="text-5xl lg:text-7xl font-bold mb-6">Showcase</h1>

            {/* Description */}
            <p className="text-xl lg:text-2xl text-white/70 max-w-4xl mx-auto mb-12 leading-relaxed">
              Watch my best and highest-rated AI breakdowns and research
              explanations. I break down complex AI concepts into digestible,
              visual content that anyone can understand and learn from.
            </p>
        ...
        <section className="px-4 pb-16 lg:pb-24">
          <div className="max-w-6xl mx-auto space-y-12">
            {/* First 6 Shorts - 3 per row */}
            <div>
              <h2 className="text-2xl font-bold text-white mb-6">
                Latest Shorts
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {showcaseVideos.slice(0, 6).map((video, index) => (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {showcaseVideos.slice(6, 8).map((video, index) => (
                  <VideoCard key={index + 6} video={video} />
                ))}
              </div>
            </div>

            {/* Last 6 Shorts - 3 per row */}
            <div>
              <h2 className="text-2xl font-bold text-white mb-6">
                More Shorts
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {showcaseVideos.slice(8, 14).map((video, index) => (
                  <VideoCard key={index + 8} video={video} />
                ))}
              </div>
            </div>
          </div>
